...
function moveMenu() {
    ...
    if (Modulos == "Limpieza") {
        ...
        prepreingresoDiesel();
    } else if (Modulos == "InsFlota") {
        app.views.main.router.navigate({ name: "yallegueFlota" });
    }
}
                        ...
                        if (item2.tipo_cedula == "checklist") {
                            ...
                            $("#pendientes").append(
                                "<li id='conc" +
                                    ...
                                    "| " +
                                    fechas[0] +
                                    "</div> <div style='color: #afafaf;font-size: 12px;margin-left: 10px;margin-top: 8px;font-weight: bold;'>Rev. Limpieza</div> </div><div class='item-after'><a href='#' onclick='continuarCed(`" +
                                    item2.id_cedula +
                                    "`,2);' style='border: none; outline:none;'><i class='material-icons md-light' style='font-size:35px;color:#00A7B5'>play_circle_outline</i></a>&nbsp;&nbsp;&nbsp;<a href='#' onclick='EliminarReg(" +
                                    item2.id_cedula +
                                    ",`" +
                                    item2.tipo_cedula +
                                    "`)' style='border: none; outline:none;'><i class='material-icons md-light' style='font-size:35px;color:red'>delete_forever</i></a></div></div></div></li>"
...
function NombreEmpresa(val) {
    ...
    if (!empresasJSON) {
        console.warn("No hay datos de empresas en localStorage");
...
function iniciarDesincorporaciones() {
    ...
    var id_cliente = localStorage.getItem("empresa");
    var nombre_cliente = NombreEmpresa(id_cliente);
...
function GuardaHeaderDesktop(id, empresa, folio, fecha, estatus, usuarioApertura, usuarioCierre, FechaApertura, OrigenApertura, OrigenCierre) {
    ...
    databaseHandler.db.transaction(
        function (tx5) {
            tx5.executeSql(
                "SELECT * FROM desincorporaciones WHERE id_servidor = ? ",
                [id],
                function (tx5, results) {
                    var length = results.rows.length;
                    if (length == 0) {
    ...
    swal({
        ...
        if (RESP == true) {
            ...
            databaseHandler.db.transaction(
                function (tx) {
                    tx.executeSql(
                        "UPDATE desincorporaciones SET fecha2  = ?, userCierre = ?, Estatus = ?, estatus_servidor = ?, OrigenCierre = ? WHERE id_cedula = ?",
...
function getIDEmpresa(nombre) {
    const empresasJSON = localStorage.getItem("empresasArray");
    if (!empresasJSON) return null;
    const empresasArray = JSON.parse(empresasJSON);
    const empresa = empresasArray.find((item) => item.Empresa === nombre);
