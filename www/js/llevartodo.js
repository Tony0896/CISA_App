                                                ...
                                                tx.executeSql("SELECT * FROM detalle_diesel WHERE id_cedula = ?",
                                                    ...
                                                    function(tx, results){
                                                        ...
                                                        for(var i = 0; i< length; i++){
                                                            ...
                                                            detalle_diesel[i] = {'Valor':i, 'id_unidad':item2.id_unidad, 'eco':item2.eco, 'eco2':item2.eco2, 'carga_total':item2.carga_total, 'odometro':item2.odometro, 'fecha_carga':fecha_carga, 'no_bomba':item2.no_bomba, 'almacen':item2.almacen, 'id_operador':item2.id_operador, 'operador':item2.operador, 'jornada':item2.jornada, 'vueltas':item2.vueltas, 'h_inicio':item2.h_inicio, 'h_fin':item2.h_fin, 'tipo_carga':item2.tipo_carga, 'operador2': item2.operador2, 'VIN': item2.VIN};
                                                        }
                                                        $.ajax({
                                                            type: "POST",
                                                            async : true,
    ...
    .then((willGoBack) => {
        if (willGoBack){
            ...
            if(tipo_cedula == "checklist"){
                ...
                databaseHandler.db.transaction(
                    function(tx){
                        tx.executeSql("DELETE FROM datos_generales_diesel WHERE id_cedula = ?",
                            [id_cedula],
                            function(tx, results){
                                databaseHandler.db.transaction(
                                    function(tx){
                                        tx.executeSql("DELETE FROM detalle_diesel WHERE id_cedula = ?",
                                            [id_cedula],
                                            function(tx, results){
                                                $("#conc" + id_cedula).remove();
                                                swal("","Eliminado correctamente","success");
